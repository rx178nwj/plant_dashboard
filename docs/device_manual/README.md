...
- **視覚フィードバック**
  ...
  - 湿度グラデーション表示（暖色=乾燥 〜 青=湿潤）
  - 温度グラデーション表示（青=5°C 〜 赤=35°C）、1秒点滅
  - 高温警告(TEMP_TOO_HIGH): 赤色0.5秒点滅
  - Bootボタン押下 → 湿度グラデーション表示
  - CTL_SW (GPIO7) 押下 → 温度グラデーション1秒点滅
  - BLE経由でのLED制御・輝度設定
  - ステータスLED（青色/赤色）
...
*   **湿度ベースLED表示:**
    ...
    *   暖色（オレンジ）= 乾燥、寒色（青）= 湿潤。
*   **温度グラデーションLED表示:**
    *   気温を色で表示（青=5°C 〜 シアン〜緑〜黄〜赤=35°C）。
    *   CTL_SW (GPIO7) 押下で温度グラデーション1秒点滅に切り替え。
    *   Bootボタン (GPIO9) 押下で湿度グラデーション固定表示に切り替え。
*   **高温警告 (TEMP_TOO_HIGH) の動作変更:**
    *   赤色LED 0.5秒点滅に変更（従来は点灯固定）。
    *   Bootボタン → 湿度グラデーション表示（警告解除）。
    *   CTL_SW → 温度グラデーション1秒点滅（警告解除）。
*   **BLE通信の機能強化:**
    *   WS2812 LED制御コマンド（CMD_CONTROL_LED, CMD_SET_LED_BRIGHTNESS）追加。
    *   センサー構成情報取得コマンド（CMD_GET_SENSOR_CONFIG）追加。
    *   直前の計測テキストログ取得コマンド（CMD_GET_LAST_LOG）追加。
...
| I2C SDA | GPIO5 | |
| I2C SCL | GPIO6 | |
| CTL_SW (制御スイッチ) | GPIO7 | ON=0, OFF=1 (active low) |
| WS2812 LED | GPIO1 | |
| 青色LED | GPIO0 | |
| 赤色LED | GPIO2 | |
| 1-Wire (DS18B20) | GPIO4 | Rev4のみ |
| Boot ボタン | GPIO9 | ESP32-C3内蔵、active low |
...
| 0x1B | CMD_GET_LAST_LOG | 直前の計測テキストログ取得 | 0 |
...
### 0x1B: CMD_GET_LAST_LOG - 直前の計測テキストログ取得

直前の60秒分析サイクルで計測されたセンサーデータと植物状態をテキスト形式で取得します。
...
UTF-8テキスト文字列（null終端なし、最大240バイト）が返されます。

**テキスト形式（Rev3/Rev4の例）:**
```
2024/03/01 12:34
気温:25.5℃ 湿度:60.0% 照度:1000lux
土壌Cap:5.2/4.8/3.1/2.9pF
土壌温度:22.5/23.0/-/-℃
外気温:18.3℃
状態:NORMAL
```
...
| 1行目 | 計測日時 | 全Rev |
| 2行目 | 気温・湿度・照度 | 全Rev |
| 3行目 | 土壌静電容量 4ch [pF] | Rev3/Rev4 |
| 4行目 | 土壌温度（有効センサーのみ数値、未接続は `-`） | Rev3/Rev4 |
| 5行目 | 外気温 (DS18B20、有効時のみ表示) | Rev4 |
| 最終行 | 植物状態（NORMAL / TEMP_TOO_HIGH 等） | 全Rev |

**エラー時**: `status_code = 0x01`、`data_length = 0`（データバッファが空の場合）

**Pythonでの受信例:**
```python
resp = await send_command(0x1B)
if resp["status"] == 0x00:
    log_text = resp["data"].decode("utf-8")
    print(log_text)
```
...
class PlantMonitor:
    ...
    async def get_last_log(self):
        """直前の計測テキストログ取得"""
        resp = await self.send_command(0x1B)
